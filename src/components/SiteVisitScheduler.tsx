  ...
  if (status === "done") {
    return (
      <div className="rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-50 p-7 text-center shadow-sm">
        ...
        <p className="font-display font-bold text-lg mb-1 text-blue-900">Visit Requested!</p>
  ...
  return (
    <div className="rounded-2xl border border-line bg-white shadow-soft overflow-hidden">
      ...
      <div className="px-6 py-4 flex items-center gap-3"
        style={{ background: "linear-gradient(135deg, #0D2050 0%, #1A3FA8 100%)" }}>
        <div className="w-9 h-9 rounded-xl bg-white/10 flex items-center justify-center shrink-0">
          <CalendarDays className="w-4 h-4 text-white" />
        </div>
        <div>
          <h3 className="font-display font-bold text-white text-base leading-tight">Schedule a Site Visit</h3>
          <p className="text-blue-300 text-xs">Free guided visit with our team</p>
        </div>
      </div>
        ...
        <button onClick={confirmVisit} disabled={status === "loading"}
          ...
          style={{
            background: slot && date && name
              ? "linear-gradient(135deg, #0D2050 0%, #1A3FA8 100%)"
              : "#E2E8F0",
            color: slot && date && name ? "white" : "#94A3B8",
            boxShadow: slot && date && name ? "0 4px 16px rgba(26,63,168,0.3)" : "none",
          }}>
          {status === "loading"
