...
function propertyIcon() {
  ...
  return L.divIcon({ html: svg, className: "", iconSize: [36, 54], iconAnchor: [18, 54], popupAnchor: [0, -56] });
}
...
export default function PropertyMap({
  ...
  return (
    <div className="relative w-full rounded-2xl overflow-hidden border border-line shadow-sm" style={{ height: "420px" }}>
          ...
          <div className="space-y-1">
            {pins.slice(0, 5).map((p, i) => (
              <div key={i} className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full shrink-0" style={{ background: getColor(p.category ?? "default") }} />
